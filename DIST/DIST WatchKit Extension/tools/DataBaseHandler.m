   ...
   int result = sqlite3_prepare(dbPoint, [sqlStr UTF8String], -1, &stmt, NULL);
    ...
    if (result == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            NSInteger begin = sqlite3_column_int(stmt, 3);
            classtable.beginWeek = @(begin);
            
            NSInteger end = sqlite3_column_int(stmt, 4);
            classtable.endWeek = @(end);
            ...
            const unsigned char *teacher = sqlite3_column_text(stmt, 8);
            classtable.teacher = [NSString stringWithUTF8String:(const char *)teacher];
